...
Companion snapshot: `run_n50_2026-05-23_mild_drift/` runs the same seed
...
**Final policy: 95-day training cadence × LMP-only (no Houston tolling, no BESS) → mean profit $148,106.34M / 6 months across 50 paths.**

- Phase A: 30d wins both stages; cadence-vs-cadence gaps remain ~$3B, dwarfing procurement gaps.
- Phase C: Gross Houston-toll option value (LP-derived, at full 100 MW reservation) = **$2.209M / 6mo** — well below the $4.8M default capacity payment ($8/kW-mo × 100 MW × 6 mo), so LMP-only wins. The toll value is independently corroborated by `ltemry/FTG-Final-Project`'s $1.42M HH-pricing estimate (~20% gap due to scope of cost calculation + price-proxy differences).
...
| Forward-curve drift | **gas +0.0 %, power +0.0 % (this run is the baseline)** | EIA May-2026 STEO has HH 2026 ≈ flat vs 2025. The mild-drift companion run is in `run_n50_2026-05-23_mild_drift/`. |
